...
export default function Game ({navigation, route}){
    ...
    function ButtomVirarOrProximo(){
        ...
        if(copia2.length == Indice + 1 && Finalizar == true){
            return(
                <TouchableHighlight style={style.BottomFinalizar} onPress={() => {setFinalizar(false); navigation.pop()}}>
        ...
        if(Prox){
            return(
                <TouchableHighlight style={style.BottomVirar} onPress={props => {
                    setProx(false);
                    setIndice(Indice + 1);
                    setFinalizar(false);
                }}>
    ...
    function Answer(){
        ...
        if(Prox) {
            return(
                <View>
                    <Text>{copia2[Indice].frente}</Text>
                </View>
            )
        }
        return(null);
    ...
    return (
        <View style={style.container}>
            <Text style={style.Cards} color="white">Cartão {Indice + 1} / {copia2.length}</Text>
        
            <View style={style.box}>
                <Text style={style.word}>{copia2[Indice].verso}</Text>
                <Answer />
            </View>
